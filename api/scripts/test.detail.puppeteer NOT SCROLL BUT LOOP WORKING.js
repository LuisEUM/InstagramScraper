...
module.exports.test = async (username) => {
    ...
    const elem = await page.$('button._a9--')
    const boundingBox = await elem.boundingBox();
    await page.mouse.move(
      boundingBox.x + boundingBox.width / 2, // x
      boundingBox.y + boundingBox.height / 2, // y
      console.log(boundingBox.y, boundingBox.x, boundingBox.width, boundingBox.height )
    ...
    await page.mouse.click(      boundingBox.x + boundingBox.width / 2, // x
    boundingBox.y + boundingBox.height / 2, // y
    console.log(boundingBox.y, boundingBox.x, boundingBox.width, boundingBox.height )

    );
    // console.log( 'Wating for the popup')
    // const popup ='.notranslate'
    // await page.waitForSelector(popup);

    // const followersDialog = '._aano';
    // await scrollDown(followersDialog, page, followers);

    // OTRAS COSAS
    // Extract the results from the page.
    // const fullFollowersList = await page.evaluate(followersList => {
    //         return [...document.querySelectorAll(followersList)].map(anchor => {
    ...
    // Print all the files.
    // console.log(fullFollowersList.join('\n'));  
